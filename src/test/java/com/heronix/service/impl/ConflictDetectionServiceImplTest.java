...
class ConflictDetectionServiceImplTest {
    ...
    void testDetectAllConflicts_WithValidSchedule_ShouldReturnStringList() {
        ...
        // Should contain emoji indicators (🔴 for HIGH severity)
    ...
    void testDetectCapacityViolations_WithOverCapacity_ShouldReturnConflict() {
        ...
        // Ensure room capacity is properly set
        testRoom.setCapacity(30);
        testSlot1.setRoom(testRoom);

        when(scheduleSlotRepository.findByScheduleId(1L)).thenReturn(Arrays.asList(testSlot1));

        // Debug: Verify test setup
        assertEquals(35, testSlot1.getEnrolledStudents());
        assertEquals(30, testSlot1.getRoom().getCapacity());
        assertTrue(testSlot1.getEnrolledStudents() > testSlot1.getRoom().getCapacity());

        List<Conflict> result = service.detectConflicts(testSchedule);

        assertNotNull(result);
        // Capacity violations should be detected when enrolled (35) > capacity (30)
        // Note: This test fails, indicating a potential bug in detectCapacityViolations
        // The direct hasCapacityConflict method works correctly
        assertTrue(result.stream().anyMatch(c -> "CAPACITY_CONFLICT".equals(c.getConflictType())),
            "Expected CAPACITY_CONFLICT. Enrolled=" + testSlot1.getEnrolledStudents() +
            ", Capacity=" + testSlot1.getRoom().getCapacity() +
