...
public class TeacherRoomPreferencesDialogController {
    ...
    private void loadExistingPreferences() {
        ...
        // Set preference type
        if (prefs.isRestrictedToRooms()) {
            restrictionRadioButton.setSelected(true);
        } else {
            preferenceRadioButton.setSelected(true);
        }

        // Set strength
        strengthComboBox.setValue(prefs.getStrength().toString());
    }
    ...
    private void populateRoomList() {
        ...
        RoomPreferences existingPrefs = teacher.getRoomPreferences();
        List<Long> selectedRoomIds = existingPrefs != null && existingPrefs.hasRooms()
                ? existingPrefs.getPreferredRoomIds()
                : new ArrayList<>();
