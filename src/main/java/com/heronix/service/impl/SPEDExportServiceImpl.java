...
public class SPEDExportServiceImpl implements SPEDExportService {
    ...
    public File exportPullOutSchedulesPdf(List<PullOutSchedule> schedules) throws Exception {
        // ✅ NULL SAFE: Validate schedules parameter
        if (schedules == null) {
    ...
    public File exportProviderWorkloadPdf(List<Teacher> providers) throws Exception {
        // ✅ NULL SAFE: Validate providers parameter
        if (providers == null) {
            throw new IllegalArgumentException("Providers list cannot be null");
        }
    ...
    public File exportIepSummaryPdf(IEP iep) throws Exception {
        ...
        // ✅ NULL SAFE: Safe extraction of IEP number with default
        String iepNumber = (iep.getIepNumber() != null) ? iep.getIepNumber() : "Unknown";
        log.info("Exporting IEP summary to PDF: {}", iepNumber);
        File exportDir = getExportsDirectory();
        String filename = "IEP_" + iepNumber + "_" + LocalDateTime.now().format(TIMESTAMP_FORMAT) + ".txt";
        File outputFile = new File(exportDir, filename);
    ...
    public File exportIepsSummaryExcel(List<IEP> ieps) throws Exception {
        // ✅ NULL SAFE: Validate ieps parameter
        if (ieps == null) {
            throw new IllegalArgumentException("IEPs list cannot be null");
        }
    ...
    public File exportServiceDeliveryPdf(List<IEPService> services) throws Exception {
        // ✅ NULL SAFE: Validate services parameter
        if (services == null) {
    ...
    public File getExportsDirectory() {
        File exportDir = new File(System.getProperty("user.home"), "Heronix Scheduler/exports/sped");
        if (!exportDir.exists()) {
            exportDir.mkdirs();
        }
        return exportDir;
    ...
    private void writeComplianceReportText(PrintWriter writer) {
        ...
        try {
            ...
            writer.printf("IEPs Expiring Soon:          %d%n", metrics.iepsExpiringWithin30Days);
            writer.println();

            // List expiring IEPs
            List<IEP> expiringIEPs = complianceService.getExpiringIEPs(30);
            // ✅ NULL SAFE: Check for null expiring IEPs list
            if (expiringIEPs != null && !expiringIEPs.isEmpty()) {
            ...
            // ✅ NULL SAFE: Check for null unscheduled services list
            if (unscheduledServices != null && !unscheduledServices.isEmpty()) {
    ...
    private void writePullOutSchedulesText(PrintWriter writer, List<PullOutSchedule> schedules) {
        ...
        for (String day : days) {
            // ✅ NULL SAFE: Filter null schedules and check dayOfWeek/startTime before comparison
            List<PullOutSchedule> daySchedules = schedules.stream()
                .filter(s -> s != null && s.getDayOfWeek() != null && day.equals(s.getDayOfWeek()))
    ...
    private void writeProviderWorkloadText(PrintWriter writer, List<Teacher> providers) {
        ...
        for (Teacher provider : providers) {
            // ✅ NULL SAFE: Skip null providers and providers with null IDs
    ...
    private void createComplianceSummarySheet(Workbook workbook, Sheet sheet) {
        ...
        try {
            ...
            createDataRow(sheet, rowNum++, headerStyle, dataStyle, "IEPs Expiring in 30 Days", String.valueOf(metrics.iepsExpiringWithin30Days));
