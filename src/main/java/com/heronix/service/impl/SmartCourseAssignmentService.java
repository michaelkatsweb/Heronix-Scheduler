...
public class SmartCourseAssignmentService {
    ...
    private int calculateMatchScore(Course course, Teacher teacher,
        ...
        boolean isCertifiedForCourse = teacher.hasCertificationForSubjectAndGrade(course.getSubject(), gradeLevel);
        ...
        if (isCertifiedForCourse && teacher.hasExpiringCertifications()) {
    ...
    private int getCourseLoad(Teacher teacher) {
        ...
        if (courses == null) {
            log.warn("Repository returned null for teacher {} courses", teacher.getId());
            return 0;
        }
