...
import com.heronix.model.ConflictResolutionSuggestion.ResolutionAction;
...
public class ConflictResolutionSuggestionServiceImpl implements ConflictResolutionSuggestionService {
    ...
    public List<ConflictResolutionSuggestion> generateSuggestions(Conflict conflict) {
        ...
        try {
            ...
            if (ollamaAIService != null && ollamaAIService.isOllamaAvailable()) {
                suggestions = enhanceSuggestionsWithAI(conflict, suggestions);
            }
    ...
    private List<ConflictResolutionSuggestion> enhanceSuggestionsWithAI(Conflict conflict, List<ConflictResolutionSuggestion> suggestions) {
        try {
            String prompt = buildAIPrompt(conflict, suggestions);
            Optional<String> aiResponseOpt = ollamaAIService.generate(prompt);
            ...
            String aiResponse = aiResponseOpt.get();

            // Parse AI response and enhance suggestions
            // For Beta 1, we log the response and use it to adjust confidence levels
            log.info("AI enhancement response: {}", aiResponse);

            // Adjust confidence based on AI feedback
            if (aiResponse.toLowerCase().contains("recommend") || aiResponse.toLowerCase().contains("best")) {
                if (!suggestions.isEmpty()) {
                    ConflictResolutionSuggestion top = suggestions.get(0);
                    top.setConfidenceLevel(Math.min(95, top.getConfidenceLevel() + 10));
                    top.setSuccessProbability(Math.min(95, top.getSuccessProbability() + 10));
                }
            }

        } catch (Exception e) {
            log.warn("AI enhancement failed, using base suggestions: {}", e.getMessage());
        }

        return suggestions;
    }

    private String buildAIPrompt(Conflict conflict, List<ConflictResolutionSuggestion> suggestions) {
        StringBuilder prompt = new StringBuilder();
        prompt.append("Analyze this scheduling conflict and provide recommendations:\n\n");
        prompt.append("Conflict Type: ").append(conflict.getConflictType()).append("\n");
        prompt.append("Severity: ").append(conflict.getSeverity()).append("\n");
        prompt.append("Description: ").append(conflict.getDescription()).append("\n\n");
        prompt.append("Generated Suggestions:\n");

        for (int i = 0; i < suggestions.size(); i++) {
            ConflictResolutionSuggestion s = suggestions.get(i);
            prompt.append(String.format("%d. %s (Impact: %d, Success: %d%%)\n",
                i + 1, s.getDescription(), s.getImpactScore(), s.getSuccessProbability()));
        }

        prompt.append("\nWhich suggestion do you recommend and why? Keep response brief.");
    ...
    private boolean applyTeacherChange(Long slotId, String newTeacherId) {
        try {
            ...
            Optional<Teacher> teacherOpt = teacherRepository.findById(Long.parseLong(newTeacherId));
