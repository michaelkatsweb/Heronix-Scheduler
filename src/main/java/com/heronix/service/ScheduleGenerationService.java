...
public class ScheduleGenerationService {
    ...
    public Schedule generateSchedule(ScheduleGenerationRequest request,
        ...
        try {
            ...
            try {
                ...
                com.heronix.model.dto.ScheduleDiagnosticReport diagnosticReport =
    ...
    /**
     * Load all teachers from database with courses eagerly loaded
     * Prevents LazyInitializationException during schedule generation
     */
    private List<Teacher> loadTeachers() {
        List<Teacher> teachers = teacherRepository.findActiveTeachersWithCourses();  // Only active teachers
        log.debug("Loaded {} active teachers with courses", teachers.size());
        return teachers;
    }

    /**
     * Load active courses from database with students eagerly loaded
     * Prevents LazyInitializationException during schedule generation
     */
    private List<Course> loadCourses() {
        List<Course> courses = courseRepository.findActiveCoursesWithStudents();
        log.debug("Loaded {} active courses with students", courses.size());
        return courses;
    }
    ...
    private List<Room> loadRooms() {
        ...
        log.debug("Loaded {} rooms", rooms.size());
        return rooms;
    }

    /**
     * Load all students from database with enrolled courses (prevents N+1 queries)
     */
    private List<Student> loadStudents() {
        // Use JOIN FETCH query to load enrolledCourses eagerly
        List<Student> students = studentRepository.findAllWithEnrolledCourses();
        log.debug("Loaded {} students with enrolled courses", students.size());
        return students;
    }
    ...
    private List<ScheduleSlot> createLunchPeriodSlots(
        ...
        List<com.heronix.model.dto.LunchWave> waveAssignments =
        ...
        for (TimeSlot lunchTimeSlot : lunchTimeSlots) {
            ...
            for (com.heronix.model.dto.LunchWave wave : waveAssignments) {
     ...
     */
    private List<com.heronix.model.dto.LunchWave> assignLunchWavesByGradeLevel(
            List<Student> students,
            int waveCount) {
        ...
        List<com.heronix.model.dto.LunchCohort> cohorts = gradeCohorts.entrySet().stream()
                .map(e -> new com.heronix.model.dto.LunchCohort(e.getKey(), e.getValue()))
                .sorted((c1, c2) -> Integer.compare(c2.getSize(), c1.getSize()))
                .collect(Collectors.toList());

        // Use bin packing to assign cohorts to waves
        List<com.heronix.model.dto.LunchWave> waves =
     ...
     * @return List of lunch waves with assigned cohorts
     */
    private List<com.heronix.model.dto.LunchWave> assignLunchWavesByCohort(
            List<Student> students,
            int waveCount,
        ...
        List<com.heronix.model.dto.LunchCohort> cohorts = cohortMap.entrySet().stream()
                .map(e -> new com.heronix.model.dto.LunchCohort(e.getKey(), e.getValue()))
                .sorted((c1, c2) -> Integer.compare(c2.getSize(), c1.getSize()))
                .collect(Collectors.toList());

        // Step 3: Use bin packing to assign cohorts to waves
        List<com.heronix.model.dto.LunchWave> waves = assignCohortsToWaves(cohorts, waveCount, students.size());
     ...
     */
    private List<com.heronix.model.dto.LunchWave> assignCohortsToWaves(
            List<com.heronix.model.dto.LunchCohort> sortedCohorts,
            int waveCount,
            int totalStudents) {

        // Initialize empty waves
        List<com.heronix.model.dto.LunchWave> waves = new ArrayList<>();
        for (int i = 0; i < waveCount; i++) {
            waves.add(new com.heronix.model.dto.LunchWave(i + 1));
        }
        ...
        for (com.heronix.model.dto.LunchCohort cohort : sortedCohorts) {
            if (cohort == null) continue;

            // Find wave with smallest current size (best fit)
            com.heronix.model.dto.LunchWave bestWave = waves.stream()
                    .filter(w -> w != null)
                    .min(Comparator.comparing(com.heronix.model.dto.LunchWave::getCurrentSize))
                    .orElse(waves.get(0));
     ...
     */
    private void logWaveAssignmentResults(List<com.heronix.model.dto.LunchWave> waves) {
        log.info("📊 ========== LUNCH WAVE ASSIGNMENT RESULTS ==========");

        // ✅ NULL SAFE: Filter null waves before processing
        for (com.heronix.model.dto.LunchWave wave : waves) {
            ...
            List<com.heronix.model.dto.LunchCohort> topCohorts = wave.getCohorts().stream()
                    .filter(c -> c != null)
                    .limit(3)
                    .collect(Collectors.toList());

            for (com.heronix.model.dto.LunchCohort cohort : topCohorts) {
        ...
        double avgSize = waves.stream()
                .mapToInt(com.heronix.model.dto.LunchWave::getCurrentSize)
                .average()
                .orElse(0);
    ...
    private String buildUserFriendlyErrorMessage(Exception exception,
                                                  com.heronix.model.dto.ScheduleDiagnosticReport report) {
        ...
        if (exception.getMessage() != null &&
            ...
            if (report.getCriticalIssuesCount() > 0) {
                ...
                int count = 1;
                for (com.heronix.model.dto.ScheduleDiagnosticReport.DiagnosticIssue issue : report.getIssues()) {
                    if (issue.getSeverity() ==
                        com.heronix.model.dto.ScheduleDiagnosticReport.IssueSeverity.CRITICAL) {
