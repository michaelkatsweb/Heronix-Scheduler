...
public class ValidationUtil {
    ...
    public static List<String> validateTeacher(Teacher teacher) {
        ...
        if (teacher.getPhoneNumber() != null && !isValidPhoneNumber(teacher.getPhoneNumber())) {
        ...
        if (teacher.getMaxConsecutiveHours() <= 0) {
            errors.add("Max consecutive hours must be positive");
        }

        if (teacher.getPreferredBreakMinutes() < 0) {
            errors.add("Preferred break minutes cannot be negative");
        }
