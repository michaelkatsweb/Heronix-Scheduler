...
public class ScheduleGeneratorController {
    ...
    @Autowired
    private com.heronix.ui.dialog.FixViolationsDialog fixViolationsDialog;
    ...
    private void loadResourceCounts() {
        ...
        try {
            resourceTeacherCount = teacherRepository != null ? teacherRepository.countByActiveTrue() : 0;  // Only active teachers
            resourceCourseCount = courseRepository != null ? courseRepository.count() : 0;
            resourceRoomCount = roomRepository != null ? roomRepository.count() : 0;
            resourceStudentCount = studentRepository != null ? studentRepository.count() : 0;
    ...
    private void showConstraintViolationDialog(String errorMessage) {
        ...
        if (result.isPresent()) {
            if (result.get() == fixIssuesBtn) {
                ...
                fixViolationsDialog.showDialog(scheduleNameField.getScene().getWindow());
    ...
    private double calculateTeacherUtilization(List<ScheduleSlot> slots) {
        try {
            ...
            List<Teacher> allTeachers = teacherService.getAllActiveTeachers();
