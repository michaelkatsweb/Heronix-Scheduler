...
public class RoomsController {
    ...
    @Autowired
    private com.heronix.repository.TeacherRepository teacherRepository;
    ...
    private GridPane createRoomForm(Room room) {
        ...
        ComboBox<com.heronix.model.domain.Teacher> teacherCombo = new ComboBox<>();
        teacherCombo.setPromptText("Unassigned");

        // Load teachers from database
        try {
            List<com.heronix.model.domain.Teacher> teachers = teacherRepository.findAllActive();
            ...
            teacherCombo.setButtonCell(new javafx.scene.control.ListCell<>() {
                @Override
                protected void updateItem(com.heronix.model.domain.Teacher teacher, boolean empty) {
        ...
        if (!isEdit && districtSettingsService != null) {
            numberField.textProperty().addListener((obs, oldVal, newVal) -> {
                if (newVal != null && !newVal.trim().isEmpty()) {
                    try {
                        String generatedPhone = districtSettingsService.generateRoomPhoneNumber(newVal);
    ...
    private Room extractRoomFromForm(GridPane grid, Room existing) {
        ...
        room.setCurrentTeacher(((ComboBox<com.heronix.model.domain.Teacher>) grid.getChildren().get(23)).getValue()); // Teacher assignment
