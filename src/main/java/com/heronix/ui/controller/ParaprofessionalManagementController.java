...
/**
 * Controller for Paraprofessional Management UI
 * Manages paraprofessional substitutes and temporary assignments
 *
 ...
 */
@Controller
public class ParaprofessionalManagementController {

    private static final Logger logger = LoggerFactory.getLogger(ParaprofessionalManagementController.class);
    ...
    // Quick Stats Labels
    @FXML private Label activeParaSubsLabel;
    ...
    public void initialize() {
        logger.info("Initializing Paraprofessional Management UI Controller");
    ...
    private void setupReportsTab() {
        reportTypeComboBox.setItems(FXCollections.observableArrayList(
                "Para Usage Summary",
                "Para Assignments by Date Range",
                "Para Hours Report",
                "Para Cost Analysis"
        ));
    ...
    private void loadQuickStats() {
        try {
            // Get all paraprofessional assignments
            List<SubstituteAssignment> allAssignments = substituteManagementService.getAllAssignments()
                    .stream()
                    .filter(a -> a.getReplacedStaffType() == StaffType.PARAPROFESSIONAL)
                    .collect(Collectors.toList());

            // Active para substitutes
            long activeParaSubs = substituteManagementService.getAllSubstitutes()
                    .stream()
                    .filter(s -> Boolean.TRUE.equals(s.getActive()) && s.getType() != null &&
                            s.getType().name().contains("PARA"))
                    .count();
            activeParaSubsLabel.setText(String.valueOf(activeParaSubs));
    ...
    /**
     * Load assignments filtered by paraprofessional staff type
     */
    private void loadAssignments() {
        try {
            List<SubstituteAssignment> allAssignments = substituteManagementService.getAllAssignments();

            // Filter for paraprofessional assignments only
            List<SubstituteAssignment> paraAssignments = allAssignments.stream()
                    .filter(a -> a.getReplacedStaffType() == StaffType.PARAPROFESSIONAL)
                    .collect(Collectors.toList());

            assignmentsTable.setItems(FXCollections.observableArrayList(paraAssignments));
            assignmentCountLabel.setText("Total: " + paraAssignments.size() + " assignments");

            // Load para substitutes for filter
            List<Substitute> paraSubs = substituteManagementService.getAllSubstitutes()
                    .stream()
                    .filter(s -> s.getType() != null && s.getType().name().contains("PARA"))
                    .collect(Collectors.toList());

            filterSubstituteComboBox.setItems(FXCollections.observableArrayList(paraSubs));
            ...
            logger.error("Error loading paraprofessional assignments", e);
            showError("Error", "Failed to load assignments: " + e.getMessage());
        }
    }

    /**
     * Load paraprofessional substitutes
     */
    private void loadSubstitutes() {
        try {
            List<Substitute> allSubstitutes = substituteManagementService.getAllSubstitutes();

            // Filter for paraprofessional substitutes only
            List<Substitute> paraSubs = allSubstitutes.stream()
                    .filter(s -> s.getType() != null && s.getType().name().contains("PARA"))
                    .collect(Collectors.toList());

            substitutesTable.setItems(FXCollections.observableArrayList(paraSubs));
            substituteCountLabel.setText("Total: " + paraSubs.size() + " para substitutes");

        } catch (Exception e) {
            logger.error("Error loading paraprofessional substitutes", e);
    ...
    private void handleNewAssignment() {
        try {
            ...
            // controller.setStaffTypeFilter(StaffType.PARAPROFESSIONAL);

            Stage stage = new Stage();
            stage.setTitle("New Paraprofessional Assignment");
    ...
    private void handleAddSubstitute() {
        Dialog<Substitute> dialog = createParaSubstituteDialog(null);
        dialog.showAndWait().ifPresent(substitute -> {
            try {
                Substitute saved = substituteManagementService.saveSubstitute(substitute);
                loadSubstitutes();
                loadQuickStats();
                showInfo("Success", "Paraprofessional substitute '" + saved.getFullName() + "' added successfully.");
            } catch (Exception e) {
    ...
    private Dialog<Substitute> createParaSubstituteDialog(Substitute existingSub) {
        Dialog<Substitute> dialog = new Dialog<>();
        dialog.setTitle(existingSub == null ? "Add Paraprofessional Substitute" : "Edit Paraprofessional Substitute");
        dialog.setHeaderText(existingSub == null ? "Enter paraprofessional substitute information" : "Modify paraprofessional substitute information");
        ...
        typeCombo.setValue(SubstituteType.PARAPROFESSIONAL);
    ...
    private void handleGenerateReport() {
        ...
        fileChooser.setTitle("Save Paraprofessional Report");
        fileChooser.setInitialFileName("para_report_" + LocalDate.now() + ".csv");
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(file))) {
            ...
            writer.println("Paraprofessional " + reportType);
