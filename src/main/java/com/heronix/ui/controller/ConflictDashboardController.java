...
public class ConflictDashboardController {
    ...
    private com.heronix.service.ConflictResolutionSuggestionService aiSuggestionService;
    ...
    @FXML private ListView<com.heronix.model.ConflictResolutionSuggestion> aiSuggestionsListView;
    ...
    private void loadAISuggestions(Conflict conflict) {
        ...
        Task<List<com.heronix.model.ConflictResolutionSuggestion>> task = new Task<>() {
            @Override
            protected List<com.heronix.model.ConflictResolutionSuggestion> call() {
                return aiSuggestionService.generateSuggestions(conflict);
            }
        };

        task.setOnSucceeded(event -> {
            List<com.heronix.model.ConflictResolutionSuggestion> suggestions = task.getValue();
            ...
            aiSuggestionsListView.setCellFactory(lv -> new ListCell<>() {
                @Override
                protected void updateItem(com.heronix.model.ConflictResolutionSuggestion item, boolean empty) {
                    super.updateItem(item, empty);
                    if (empty || item == null) {
                        setText(null);
                        setStyle("");
    ...
    private void loadPriorityScore(Conflict conflict) {
        ...
        Task<com.heronix.model.ConflictPriorityScore> task = new Task<>() {
            @Override
            protected com.heronix.model.ConflictPriorityScore call() {
                return aiSuggestionService.calculatePriorityScore(conflict);
            }
        };

        task.setOnSucceeded(event -> {
            com.heronix.model.ConflictPriorityScore score = task.getValue();
            if (score != null) {
     ...
     */
    private void displayAISuggestionDetails(com.heronix.model.ConflictResolutionSuggestion suggestion) {
        if (suggestion == null) {
            clearAISuggestionDetails();
            return;
        }
    ...
    private void handleApplyAISuggestion() {
        ...
        com.heronix.model.ConflictResolutionSuggestion selectedSuggestion =
     ...
     */
    private void applyAISuggestion(com.heronix.model.ConflictResolutionSuggestion suggestion) {
        setLoading(true);
    ...
    private void handleViewAIDetails() {
        ...
        if (selectedSuggestion.getActions() != null) {
            for (com.heronix.model.ConflictResolutionSuggestion.ResolutionAction action : selectedSuggestion.getActions()) {
