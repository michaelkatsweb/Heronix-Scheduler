...
import com.heronix.ui.components.TimePickerField;
...
public class EventsController {
    ...
    private void handleExport() {
        ...
        try {
            ...
            if (file != null) {
                byte[] data = exportService.exportEventsToICal(eventsList);
    ...
    private void showEventDialog(Event event) {
        ...
        TimePickerField startTimeField = new TimePickerField(java.time.LocalTime.of(9, 0));
        if (isEditMode && event.getStartDateTime() != null) {
            startTimeField.setTime(event.getStartDateTime().toLocalTime());
        }

        TimePickerField endTimeField = new TimePickerField(java.time.LocalTime.of(10, 30));
        ...
        startTimeField.timeProperty().addListener((obs, old, newVal) -> validateForm.run());
        endTimeField.timeProperty().addListener((obs, old, newVal) -> validateForm.run());
