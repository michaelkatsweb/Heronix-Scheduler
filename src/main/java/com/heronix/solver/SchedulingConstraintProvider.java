...
public class SchedulingConstraintProvider implements ConstraintProvider {
    ...
    private Constraint teacherAvailability(ConstraintFactory constraintFactory) {
        return constraintFactory
                ...
                .filter(slot -> {
                    ...
                    boolean isAvailable = teacher.isAvailableAt(
    ...
    private Constraint teacherRoomPreferences(ConstraintFactory constraintFactory) {
        return constraintFactory
                ...
                .penalize(HardSoftScore.ONE_SOFT, (slot) -> {
                    Teacher teacher = slot.getTeacher();
                    com.heronix.model.dto.RoomPreferences prefs = teacher.getRoomPreferences();

                    // Get penalty weight based on preference strength
                    int penaltyWeight = prefs.getStrength().getPenaltyWeight();
                    ...
                    if (multiRoomCourse.getRoomAssignments() != null) {
                        return multiRoomCourse.getRoomAssignments().stream()
                                ...
                                .anyMatch(assignment ->
                                    ...
                                    assignment.getRoom().getId().equals(conflictingRoom.getId()));
                    }
    ...
    private Constraint multiRoomProximity(ConstraintFactory constraintFactory) {
        return constraintFactory
                ...
                .penalizeLong(HardSoftScore.ONE_SOFT, slot -> {
                    ...
                    for (com.heronix.model.domain.CourseRoomAssignment assignment : course.getRoomAssignments()) {
                        if (!Boolean.TRUE.equals(assignment.getActive()) || assignment.getRoom() == null) {
                            continue;
                        }

                        Room otherRoom = assignment.getRoom();
        ...
        return constraintFactory
                ...
                .penalizeLong(HardSoftScore.ONE_SOFT, slot -> {
                    ...
                    if (course.getRoomAssignments() != null) {
                        for (com.heronix.model.domain.CourseRoomAssignment assignment : course.getRoomAssignments()) {
                            if (Boolean.TRUE.equals(assignment.getActive()) && assignment.getRoom() != null) {
                                Integer roomCapacity = assignment.getRoom().getCapacity();
